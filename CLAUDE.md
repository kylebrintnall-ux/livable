...
- [x] Round 10.2: Loading state (AnalysisScreen self-manages summary via useEffect — navigates immediately, HouseLoader in summary area while API pending; "EXPORT AS PDF" button disabled during load); chip-row LegendChips in MapScreen (compact horizontal flex-wrap replacing tall vertical Legend — reclaims ~200px for treemap); icon-only fallback on lifestyle tiles below all size thresholds; PDF font vendored to public/fonts/jost/ (eliminates jsDelivr network dep, __dirname moved before Font.register); page fontFamily switched to Jost; AnalysisScreen sticky footer (flex-column layout, scrollable content + pinned action row above safe-area-bottom); button copy changed "Share This ↗" → "EXPORT AS PDF ↗"; MapScreen analyzing/analyzeError states removed (navigation is now instant).
